    ...
    <div className="min-h-screen bg-slate-950 p-6 sm:p-12 text-slate-100">
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Secure Neural Node Management • spider</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-slate-900 border-slate-800 shadow-2xl rounded-[2rem]">
        ...
        <Card className="bg-slate-900 border-slate-800 shadow-2xl rounded-[2.5rem] overflow-hidden">
          <CardHeader className="p-8 border-b border-slate-800">
            ...
            <CardDescription className="text-slate-400">Real-time synchronization with Firebase Firestore</CardDescription>
          </CardHeader>
                ...
                {users && users.length > 0 ? (
                  users.map((u: any) => (
                    <TableRow key={u.id} className="border-slate-800 hover:bg-slate-800/30 transition-colors">
                      ...
                      <TableCell className="font-mono text-[10px] text-slate-500 py-4 px-8">{u.id}</TableCell>
                      <TableCell className="text-right py-4 px-8">
                        <Badge variant="outline" className={cn(
                          "text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-lg",
                          u.role === 'Admin' ? "border-primary/20 text-primary bg-primary/5" : "border-slate-700 text-slate-400"
                        )}>
                          {u.role || 'User'}
                        </Badge>
                      </TableCell>
